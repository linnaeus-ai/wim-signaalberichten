"""
Dual-head multi-label PyTorch training script for Dutch BERT models.
...
Default model: dmBERT-Dutch (512 hidden, 22 layers, vocab size 32128).
"""
...
class mmBERTDualHead(nn.Module):
    ...
    def __init__(self, model_name, num_onderwerp, num_beleving, dropout, initial_threshold, use_thresholds: bool = True):
        ...
        # Shared BERT encoder (architecture depends on model_name)
        self.encoder = AutoModel.from_pretrained(model_name)
        hidden_size = self.encoder.config.hidden_size  # 512 for dmBERT-Dutch, 768 for mmBERT-base
    ...
    def forward(self, input_ids, attention_mask):
        # Get shared representation from BERT encoder
        ...
        # Works for both models with/without pooler_output
...
def create_argument_parser():
    parser = argparse.ArgumentParser(description="Train BERT dual-head model on Wim dataset with wandb sweeps.")
    parser.add_argument(
        "--excel_file_path",
        type=str,
        required=True,
...
def main():
    ...
    model_name = "yhavinga/dmbert-dutchl-512h-22l-1350000"
    ...
    print(f"\nLoading tokenizer and creating dual-head model from: {model_name}...")
    ...
    print("\n" + "=" * 60)
    print("Training complete! 🎉")
    print(f"BERT dual-head architecture with combined F1+BCE loss")
